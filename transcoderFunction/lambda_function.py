...
def get_s3_metadata(bucket, key):
    print(f"Getting metadata for: {bucket}/{key}")
...
def lambda_handler(event, context):
    ...
    max_speaker_labels = media_metadata.get('maxspeakerlabels', '1')

    print(f"Dowloading video file to efs: {efsPath}/{videoFileName}")
    ...
    print(f"Calling subprocess ffmpeg with {efsPath}/{videoFileName} to {efsPath}/{soundFileName}.mp3")
    subprocess.call(['/opt/bin/ffmpeg', '-loglevel', 'error', '-i', f"{efsPath}/{videoFileName}", '-vn',
                     f"{efsPath}/{soundFileName}.mp3"])
    with open(f"{efsPath}/{soundFileName}.mp3", 'rb') as f:
        data = f.read()
        print("Reading transformed file")
        s3.put_object(Bucket=audioOutputBucket, Key=f"{soundFileName}.mp3", Body=data,
                      Metadata={'languagecode': language_code,
                                'channelidentification': channel_identification,
                                'maxspeakerlabels': max_speaker_labels})

        print(
            f"Put audio output to {audioOutputBucket} with {soundFileName}.mp3 and metadata:"
            f" {language_code},{channel_identification},{max_speaker_labels} ")
    print("Cleaning up efs temp files...")
    os.remove(f"{efsPath}/{soundFileName}.mp3")
    os.remove(f"{efsPath}/{videoFileName}")
